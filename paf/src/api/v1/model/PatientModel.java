...
public class PatientModel {
	...
	public Map<String, Object> insertPatient(Patient p) {
		...
		try {
			...
			preparedStmt.setInt(4, p.getGender()); // binding values
			...
			preparedStmt.setInt(9, p.getMarital_status()); // binding values
			preparedStmt.setInt(10, p.getActivation()); // binding values
	...
	public Map<String, Object> updatePatient(Patient p) {
		...
		try {
			...
			preparedStmt.setInt(3, p.getGender()); // binding values
			...
			preparedStmt.setInt(8, p.getMarital_status()); // binding values
			preparedStmt.setInt(9, p.getId()); // binding values
	...
	public Map<String, Object> deletePatient(int id) {
		...
		try {
			// Set and execute query
			PreparedStatement preparedStmt = con.prepareStatement(query);
			preparedStmt.setInt(1, id); // binding values
			preparedStmt.executeUpdate();
	...
	// |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
	// To Delete A Patient------------------------------------------------------------
	public Map<String, Object> deleteFlagePatient(int id) {
	...
	// |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
	// To Get A Patient------------------------------------------------------------
	public Map<String, Object> getPatient(int id) {
		...
		try {
			...
			while (rs.next()) {
				...
				p.setMarital_status(rs.getInt(13));
				p.setActivation(rs.getInt(14));
			}
